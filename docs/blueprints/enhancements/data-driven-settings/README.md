...
- **Downstream consumer impact: none.** A consumer's integration controller hooks only the public filters `bunnify_allow_non_upload_url` and `bunnify_skip_image`; it never reads `bunnify_enabled` or any settings option directly (a grep across the consumer's plugins returns no option reads). The filter API in `docs/HOOKS.md` is untouched.
